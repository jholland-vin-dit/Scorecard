...
$sql_main_table = pg_query($link, $str_main_table);
$row_main_table = pg_fetch_assoc($sql_main_table);
...
$sql_name_of_body = pg_query($link, $str_name_of_body);
$row_name_of_body = pg_fetch_assoc($sql_name_of_body);
...
$sql_main_table_columns = pg_query($link, $str_main_table_columns);
//echo $str_main_table_columns;
//$row_main_table_columns = pg_fetch_assoc($sql_main_table_columns);
...
$sql_sub_tables = pg_query($link, $str_sub_tables);
...
$sql_table_columns = pg_query($link, $str_table_columns);
...
// show table columns
while($row_main_table_columns = pg_fetch_assoc($sql_main_table_columns)){
...
while($row_sub_tables = pg_fetch_assoc($sql_sub_tables)){
...
//	echo "<tr><td>".$str_table_columns."</td></tr>";
	while($row_table_columns = pg_fetch_assoc($sql_table_columns)){
	//	echo "<tr><td valign=top bgcolor=lightgreen>&nbsp;- ".$row_table_columns["column_name"]."</td></tr>";
		...
		$sql_select_lookup = pg_query($link, $str_select_lookup);
		//	echo "<tr><td>".$str_select_lookup."</td></tr>";
		// echo "<tr><td>select * from ".$row_table_columns["name"]." where body_id=1</td></tr>";
		while($row_select_lookup = pg_fetch_assoc($sql_select_lookup)){
